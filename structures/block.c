...
extern void requires(int reqkind, char* req);
...
extern void create_variable_bytecode(identifier*, bytecode*, char*, int);
extern void create_put_variable_bytecode(identifier*, bytecode*, char*, int);
...
void load_extern_library(string* library_name)
{
	...
	string_append(global_library_path, string_from_cstr(PATH_SEP_STR "Lib_"));
	...
	string_append(project_library_path, string_from_cstr(PATH_SEP_STR "Lib_"));
	...
	if (library_file == NULL)
	{
		...
		sprintf(filename, "%s" PATH_SEP_STR "%s.bsf", output_path, string_get_cstr(library_name));
