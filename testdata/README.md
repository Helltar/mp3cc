...
`CatchRect` needs `libs/Lib_sensor.class`, a small touch-input helper compiled
from `lib_sensor.java`, resolved through the `-p` switch. Its five units must be
compiled in dependency order — `ucore`, `uplatfrm`, `urect`, `ugame`,
