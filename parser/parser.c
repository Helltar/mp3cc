...
void parser_start()
{
	...
	else
	{
		...
		sprintf(symbols_filename, "%s" PATH_SEP_STR "%s.bsf", output_path, string_get_cstr(str_program_name));
	...
	if (error_count == 0)
	{
		...
		if (compiling_unit == 0)
		{
			sprintf(output_file_name, "%s" PATH_SEP_STR "M.class", output_path);
			sprintf(short_file_name, "M");
		}
		else
		{
			sprintf(output_file_name, "%s" PATH_SEP_STR "%s.class", output_path, string_get_cstr(str_program_name));
